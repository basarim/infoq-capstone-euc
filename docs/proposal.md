...
![Context diagram: EUC Artifacts from Intent to Evaluation. Business Intent is formalized into an Executable Use Case, a machine-readable artifact containing actor, goal, rules, policies, expectedOutcomes, and evaluation criteria. The EUC drives Execution (a Runtime combining deterministic rules, retrieval, and LLM reasoning) which produces an Execution Result, and defines criteria for an Evaluation Harness (deterministic evals plus an AI judge) which assesses that result and yields an Evaluation Result that feeds back to refine intent. The same EUC artifact is consumed at every SDLC stage — Requirements, Architecture, Development, Runtime, Evaluation, and Regression — since prompts, models, and retrieval can change while the EUC stays the same business contract throughout.](images/euc-lifecycle-overview.svg)
